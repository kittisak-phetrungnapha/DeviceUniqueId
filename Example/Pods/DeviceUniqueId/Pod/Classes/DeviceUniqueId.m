...
//  Pods
//
//  Created by Kittisak Phetrungnapha on 4/7/2559 BE.
//
//  Source from https://gist.github.com/csnu17/a3a00ab16aeb5645c3199fb5228611bc
...
+ (NSString *)getUniqueId {
    return [[[DeviceUniqueId alloc] initWithKey:@"DeviceUniqueId"] uid];
}
...
/*! Returns the Device UID.
 ...
 - Keychain
 - NSUserDefaults
 - Apple IDFV (Identifier for Vendor)
...
- (NSString *)uid {
    ...
    if (!_uid) {
        _uid = [[self class] appleIDFV];
    }
    /*
    if (!_uid) {
        _uid = [[self class] appleIDFA];
    }
    */
    if (!_uid) {
 ...
 */
- (void)save {
    if (![DeviceUniqueId valueForUserDefaultsKey:_uidKey]) {
        [DeviceUniqueId setValue:_uid forUserDefaultsKey:_uidKey];
    }
    if (![DeviceUniqueId valueForKeychainKey:_uidKey service:_uidKey]) {
        [DeviceUniqueId setValue:_uid forKeychainKey:_uidKey inService:_uidKey];
    }
}
